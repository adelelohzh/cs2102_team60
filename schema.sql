...
DROP TABLE IF EXISTS Employees, Junior, Booker, Senior, Manager, Meeting_Rooms, Departments, Health_Declaration, Sessions, Works_In, Located_In, Joins, Books, Approves, Updates;
;
...
CREATE TABLE Books (
	...
	date DATE,
	PRIMARY KEY (room, floor, time, date),
	FOREIGN KEY (room) REFERENCE Sessions (room) ON DELETE CASCADE, 
	FOREIGN KEY (floor) REFERENCE Sessions (floor) ON DELETE CASCADE,
	FOREIGN KEY (time) REFERENCE Sessions (time) ON DELETE CASCADE, 
	FOREIGN KEY (date) REFERENCE Sessions (date) ON DELETE CASCADE,
    FOREIGN KEY (eid) REFERENCE Booker (eid) ON DELETE CASCADE
);
...
CREATE TABLE Approves(
	...
	FOREIGN KEY (time) REFERENCE Sessions (time) ON DELETE CASCADE, 
	FOREIGN KEY (date) REFERENCE Sessions (date) ON DELETE CASCADE,
    FOREIGN KEY (eid) REFERENCE Manager (eid) ON DELETE CASCADE
);
...
CREATE TABLE Updates (
    ...
    FOREIGN KEY (room,floor) REFERENCES Meeting Rooms ON DELETE CASCADE
 );
